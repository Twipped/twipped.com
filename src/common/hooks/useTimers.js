...
export function useTween (active, ms = 200, delay = 0) {
  const [ elapsed, setElapsed ] = useState(0);
  const startRef = useRef();
  const { start, stop, isActive } = useDeferredLoop(useEventCallback(() => {
    if (!active) return;
    const time = Math.min(1, (Date.now() - startRef.current) / ms);
    setElapsed(time);
    if (time >= 1) stop();
  }));
